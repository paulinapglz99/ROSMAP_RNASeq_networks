...
plot_metric_distribution <- function(random_metrics, observed_metric, metric_name, graph_type, ci_lower, ci_upper) {
  random_df <- data.frame(value = random_metrics, type = paste0("Random (", graph_type, ")")) # Randomized values
  observed_df <- data.frame(value = observed_metric, type = paste0("Observed (", graph_type, ")"))   #Empiric values
  ...
  ggplot(random_df, aes(x = value)) +
    ...
    geom_vline(aes(xintercept = ci_lower), color = "darkgreen", linetype = "dotted", size = 0.8) + 
    geom_vline(aes(xintercept = ci_upper), color = "darkgreen", linetype = "dotted", size = 0.8) + 
...
#Function to calculate gamma from degree distributions --- ---
...
#Calculate empiric metrics for randomized graphs
...
#95% confidence interval for clustering coefficient
...
#95% confidence interval for modularity
...
#Randomize without maintaining degree dis
...
#Calculate observed gamma values for AD and control networks ----
