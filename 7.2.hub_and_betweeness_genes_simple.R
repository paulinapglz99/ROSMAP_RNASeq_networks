...
#Percentile for each node
percentile_ecdf <- ecdf_degree(as.numeric(degree_in_control_control)) * 100  # Asegurarse de que sean números
...
degree_dif_control <- data.frame(
  ...
  degree_percentile = percentile_ecdf, 
  degree_percentile1 = (100 - percentile_ecdf), 
  degree_diff = degree_in_control_control -degree_in_AD_control
)
...
degree_dif_control <- degree_dif_control %>% 
  left_join(degree_diff_control.x) %>% 
  arrange(by = desc(degree_diff))
degree_dif$external_gene_name[7] <- "SNORD56"
degree_dif$external_gene_name[8] <- "HINT1P"
degree_dif$external_gene_name[9] <- "LOC100506405"
...
# Calcular betweenness para graphnoAD (control)
...
# Calcular betweenness para graphAD
...
# Unir los dataframes por 'ensembl_gene_id', permitiendo nodos ausentes en alguna red
merged_df <- merge(
...
merged_df.x <- convert_ens_to_symbol(merged_df$ensembl_gene_id)

merged_df<- merged_df %>% left_join(merged_df.x, by = "ensembl_gene_id")

# Reemplazar los valores NA con 0 (asumiendo que nodos ausentes tienen betweenness 0)
merged_df[is.na(merged_df)] <- 0
 
# Calcular percentiles de betweenness para cada red
merged_df$betweenness_percentile_control <- rank(merged_df$betweenness_in_control) / nrow(merged_df)
merged_df$betweenness_percentile_AD <- rank(merged_df$betweenness_in_AD) / nrow(merged_df)

# Calcular la diferencia entre los valores de betweenness
merged_df$betweenness_diff <-  merged_df$betweenness_in_control - merged_df$betweenness_in_AD

#only in control

only_bet_control <- merged_df %>% 
...
diff_be_only_control_.p <- ggplot(only_bet_control.l, aes(x = external_gene_name, y = betweenness, 
                             color = network)) +
  geom_point(size = 3) +  # Puntos para cada betweenness
  geom_segment(
    aes(x = external_gene_name, xend = external_gene_name, 
        y = 0, yend = betweenness), 
    size = 1.5
  ) +  # Segmentos que conectan cada punto con y=0
...
# Ordenar external_gene_name basado en betweenness_diff
...
geom_segment(data = subset(only_bet_AD.l, network == "AD"),
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1),  # Rotar etiquetas de eje x
    legend.position = "top"  # Leyenda en la parte superior
  ) +
  labs(
    title = "Betweenness of nodes AD-network-specific",
    x = NULL,
    y = "Betweenness",
    color = "Network"
  ) +
